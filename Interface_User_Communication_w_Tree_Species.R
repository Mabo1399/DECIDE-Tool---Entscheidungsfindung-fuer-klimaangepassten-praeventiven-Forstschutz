...
# Mixture type per forest class (pictogram confirmation question) --------------
# Combines separate fields per forest class (only 3 pictograms per question)
# with globally unique option values (otherwise the images collide across
# questions). Value scheme: <type>_<forest-class-suffix>, e.g. rein_StH.
# R derives the type from the 70% rule (ba_klasse) and prefills only the field
# matching the current forest class as the default for the pictogram selection
# (the actual prefill / non-overwrite happens below in result_list).
#
# This configuration must match the survey question aliases and option values
# exactly (ASCII, no umlauts, exact spelling):
#   - type token:          rein / mischnh / misch  (the conifer mixture
#     "nhmisch" is mapped here to "mischnh")
#   - forest-class suffix: Di / StH / swBH  (for sw Baumholz the existing
#     aliases use partly "BH" (Schaeden_BH_, Massnahmen_Tabelle_BH) and partly
#     "swBH" (Kronenlaenge_swBH); "swBH" is used consistently here, so that
#     field name, option value and condition all use the same suffix)
mischungstyp_typ_token <- c(rein = "rein", nhmisch = "mischnh", misch = "misch")
mischungstyp_wk <- list(
  "Dickung"     = list(suffix = "Di",   feld = "Mischungstyp_Di"),
  "Stangenholz" = list(suffix = "StH",  feld = "Mischungstyp_StH"),
  "sw Baumholz" = list(suffix = "swBH", feld = "Mischungstyp_swBH")
)

mischungstyp_feld  <- NULL
mischungstyp_value <- NULL
if (RUN_BA_KLASSE && !is.na(ba_klasse)) {
  .mt_wkcfg <- mischungstyp_wk[[ as.character(json_data$Wuchsklasse) ]]
  .mt_tok   <- unname(mischungstyp_typ_token[ ba_klasse ])
  if (!is.null(.mt_wkcfg) && !is.na(.mt_tok) && nzchar(.mt_tok)) {
    mischungstyp_feld  <- .mt_wkcfg$feld
    mischungstyp_value <- paste0(.mt_tok, "_", .mt_wkcfg$suffix)   # e.g. rein_StH
  }
}
...
ZIEL_HTML_INITIATOR <- TRUE
# this script is the initiator (tree-species selection step): here, for the
# first time all three inputs are present -> the first calculation happens here.
...
# only PREFILL the mixture type (default for the pictogram selection) and only
# if the user has not chosen anything yet -> a confirmed selection is
# NOT overwritten on a rerun.
if (!is.null(mischungstyp_feld) && !is.null(mischungstyp_value)) {
  .mt_existing <- json_data[[mischungstyp_feld]]
  if (is.null(.mt_existing) || length(.mt_existing) == 0 ||
      !nzchar(trimws(as.character(.mt_existing)[[1]]))) {
    result_list[[mischungstyp_feld]] <- mischungstyp_value
  }
}
